<template>
  <div>
    <div class="card border-0 shadow position-relative">
      ...
      <div class="card-body">
        <div class="auth-box">
          <span style="background-color: var(--primary-color)" class="py-1 px-3 text-white rounded-pill">{{ book.author
            }}</span>
        </div>
      ...
      <span :class="ratingBadgeClass"
        class="circleclass position-absolute top-0 start-100 translate-middle p-2 text-light rounded-circle border border-2 border-light">
        {{ book.rating }}</span>
    </div>
  </div>
</template>
